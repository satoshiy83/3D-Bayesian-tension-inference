...
function result = draw_meanTension(epithelium,balanceDict)
...
meanTensions = nan([epithelium.cells.count,1]);
for i = 1:epithelium.cells.count
    cel = epithelium.cells.objectAtIndex(i);
    meanTensions(i) = cel.meanSurfaceTension(balanceDict,resolution);
end

if ~balanceDict.isNanForKey("range_meanTension")
    range = balanceDict.objectForKey("range_meanTension");
else
    range = [min(meanTensions),max(meanTensions)];
end

for i = 1:epithelium.cells.count
    if isnan(meanTensions(i))
        continue
    end
    ...
    bitmap(nask.var) = meanTensions(i);
end
