...
function result = draw_stress_tensor_normalized(context, epithelium, ...
    balanceDict, clusters, scale_factor, lineWidth, color)
...
for i = 1:clusters.count
    ...
    startPoint.x = center(2) - v(1) * scale_factor;
    startPoint.y = center(1) - v(2) * scale_factor;
    painter.move(startPoint);
    endPoint.x = center(2) + v(1) * scale_factor;
    endPoint.y = center(1) + v(2) * scale_factor;
